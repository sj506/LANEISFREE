<template>
  <div>
    ...
    <div class="buyBox">
      ...
      <div>
        <button>{{ $addComma(price()) }}원 결제하기</button>
      </div>
    </div>
  </div>
  <div class="container">
    ...
    <div class="list">
      ...
      <div class="formBox">
        ...
        <div class="formData">
          ...
          - <input class="telInput" type="text" /> - <input class="telInput" type="text" />
        </div>
        <div class="formData">
          <label>이메일 <input class="" type="text" /></label>
        </div>
        <!-- 이메일 형식으로 -->
        <div class="formData">
          <label>배송지 선택</label>
          <label class="formData"
            ...
            <input v-if="RequireOption === '메세지 직접 입력'" type="text" placeholder="배송 요청사항을 입력해주세요.(최대 45자까지 입력)" />
          </label>
        </div>
      </div>
      <hr />
    ...
    <div class="list">
      ...
      <div>
        ...
        <input type="number" min="0" v-model="productCount" />개 {{ $addComma(price()) }}원
      </div>
      <hr />
    </div>
    <!-- ---------------------------- -->
    <div class="list">
      ...
      <span v-for="(payment, idx) in payment" :key="idx">
    ...
    <div>
      ...
      <br />
      결제 가능한 은행: NH농협, 국민, 신한, 우리, 기업, SC제일, 부산, 경남, 수협, 우체국, 미래에셋대우, 광주, 대구, 전북, 새마을금고, 제주은행, 신협,
      하나은행, 케이뱅크, 카카오뱅크, 삼성증권
      <br />
